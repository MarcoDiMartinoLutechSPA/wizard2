sap.ui.define([
    ...
    "sap/btp/wizard2/controller/BaseController",
              ...
              BaseController, Fragment, MessageBox, Filter, FilterOperator) 
    {
        ...
        return BaseController.extend("sap.btp.wizard2.controller.View1", 
        {
            ...
            onInit: function () {
                ...
                var oVisibilityModel = new JSONModel({
                    backButtonVisible: false, // Impostato inizialmente su false se desideri non visualizzarlo
            ...
            onAfterRendering: function() {
                ...
                var aSteps = this.getView().byId("stepContainer").getContent();
                for (var i = 1; i < aSteps.length; i++) {
                    aSteps[i].setVisible(false);
                }
            },

            onNextStep: async function() {
                var aSteps = this.getView().byId("stepContainer").getContent();
                var iCurrentStepIndex = aSteps.findIndex(function(oStep) {
                    return oStep.getVisible();
                });
            
                // Esegui la validazione in base allo step corrente
                switch (iCurrentStepIndex) 
                {
                    case 0: // Step1
                ...
                aSteps[iCurrentStepIndex].setVisible(false);
            
                // Mostra lo Step successivo
                if (iCurrentStepIndex < aSteps.length - 1) {
                    aSteps[iCurrentStepIndex + 1].setVisible(true);
                }
            
                // Aggiorna la visibilità dei pulsanti in base allo Step corrente
                this.updateButtonVisibility(iCurrentStepIndex + 1);
            },
            ...
            onPreviousStep: function() {
                var oStepContainer = this.getView().byId("stepContainer");
                var oWizardFragments = oStepContainer.getContent();
                var oVisibilityModel = this.getView().getModel("visibilityModel");
                var iCurrentStepIndex = -1;
            
                // Trova l'indice dello step attualmente visibile
                for (var i = 0; i < oWizardFragments.length; i++) {
                    if (oWizardFragments[i].getVisible()) {
                        iCurrentStepIndex = i;
                        break;
                    }
                }
            
                // Torna indietro allo step precedente
                if (iCurrentStepIndex > 0) {
                    oWizardFragments[iCurrentStepIndex].setVisible(false); // Nascondi lo step corrente
                    oWizardFragments[iCurrentStepIndex - 1].setVisible(true); // Mostra lo step precedente
                    this.updateButtonVisibility(iCurrentStepIndex - 1); // Aggiorna la visibilità dei pulsanti
                }
            
                // Imposta la visibilità dei pulsanti in base allo step attuale
                if (iCurrentStepIndex === 1) {
                    // Se sei nello Step2
                    oVisibilityModel.setProperty("/backButtonVisible", false);
                } else {
                    // Se sei in un altro step
            ...
            onCustomerSelectionChange: function(oEvent) {
                ...
                var oStep4Fragment = this.byId("stepContainer").getContent()[3];
                ...
                MessageBox.confirm("Confermare il suo ordine?", {
                    ...
                    onClose: function(oAction) {
                        if (oAction === MessageBox.Action.OK) {
                            ...
                            for (var i = 1; i < oWizardFragments.length; i++) {
                                oWizardFragments[i].setVisible(false);
                            }
            
                            // Mostra lo Step 1
                            oWizardFragments[0].setVisible(true);
                            
                            // Torna all'inizio del wizard utilizzando previousStep
                            that.byId("dynamicPage").getScrollDelegate().scrollTo(0, 0);
                        }
                    }
                });
            }
